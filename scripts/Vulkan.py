...
SHUNYA_VULKAN_VERSION = '1.4.341.1'
VULKAN_SDK_EXE_PATH = 'third_party/VulkanSDK/VulkanSDK.exe'
...
def CheckVulkanSDK():
    if (VULKAN_SDK is None):
        ...
        InstallVulkanPrompt()
        return False
    elif (SHUNYA_VULKAN_VERSION not in VULKAN_SDK):
        print(f"Located Vulkan SDK at {VULKAN_SDK}")
        print(f"You don't have the correct Vulkan SDK version! (Shunya requires {SHUNYA_VULKAN_VERSION})")
        InstallVulkanPrompt()
        return False
...
OutputDirectory = "third_party/VulkanSDK"
